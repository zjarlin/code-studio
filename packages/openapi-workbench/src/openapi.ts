    ...
    .flatMap(([path, pathItem]) =>
      Object.entries(pathItem ?? {})
        ...
        .map(([method, operation]) => {
          ...
          const parameters = mergeParameters(pathItem?.parameters ?? [], detail.parameters ?? [])
          ...
          return {
            ...
            summary: detail.summary?.trim() || detail.operationId?.trim() || `${method.toUpperCase()} ${path}`,
...
function mergeParameters(pathParameters: ApiOperation['parameters'], operationParameters: ApiOperation['parameters']): ApiOperation['parameters'] {
  const parameters = new Map(pathParameters.map((parameter) => [`${parameter.in}:${parameter.name}`, parameter]))
  operationParameters.forEach((parameter) => parameters.set(`${parameter.in}:${parameter.name}`, parameter))
...
export function requestBodySample(operation: ApiOperation, document: ApiDocument, selectedContentType?: string): string {
  const contentType = requestContentType(operation, selectedContentType)
  const media = contentType ? operation.requestBody?.content?.[contentType] : undefined
  if (!media) {
    return ''
  }
...
export function requestContentType(operation: ApiOperation | undefined, selectedContentType?: string): string | undefined {
  const content = operation?.requestBody?.content ?? {}
  if (selectedContentType && content[selectedContentType]) {
    return selectedContentType
  }
  return ['application/json', 'multipart/form-data', 'application/x-www-form-urlencoded']
    .find((contentType) => content[contentType]) ?? Object.keys(content)[0]
}

export function requestBodySchema(operation: ApiOperation | undefined, selectedContentType?: string): ApiSchema | undefined {
  const contentType = requestContentType(operation, selectedContentType)
...
export function requestFormFields(
  ...
  const contentType = requestContentType(operation, selectedContentType)
  if (!contentType || !['multipart/form-data', 'application/x-www-form-urlencoded'].includes(contentType)) {
    return []
  }
