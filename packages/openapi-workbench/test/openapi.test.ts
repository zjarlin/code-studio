...
import type { ApiDocument } from '../src/types'
...
describe('api studio OpenAPI projection', () => {
  ...
  it('merges path parameters with operation overrides and keeps security requirements', () => {
    const operation = collectApiOperations({
      paths: {
        '/projects/{id}': {
          parameters: [
            { name: 'id', in: 'path', required: true, description: 'path id', schema: { type: 'string' } },
            { name: 'locale', in: 'query', schema: { default: 'zh-CN' } },
          ],
          get: {
            parameters: [{ name: 'id', in: 'path', required: true, description: 'operation id', schema: { type: 'integer' } }],
            security: [{ bearerAuth: [] }],
          },
        },
      },
    ...
    expect(operation.parameters).toEqual([
      expect.objectContaining({ name: 'id', description: 'operation id', schema: { type: 'integer' } }),
