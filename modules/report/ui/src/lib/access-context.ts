...
export async function authenticatedFetch(input: RequestInfo | URL, init: RequestInit = {}): Promise<Response> {
  ...
  if (tenantId && !headers.has('tenant-id')) headers.set('tenant-id', tenantId)
  if (visitTenantId && !headers.has('visit-tenant-id')) headers.set('visit-tenant-id', visitTenantId)
