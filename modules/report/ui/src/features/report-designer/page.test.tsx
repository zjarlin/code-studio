...
import { ReportPublicationError } from '@/features/reports/operations'
import * as reportApi from '@/features/reports/operations'
...
vi.mock('@/components/composed/catalog-action/catalog-icon-action', () => ({
  CatalogIconAction: ({ elementKey, ...props }: { elementKey: string } & ButtonHTMLAttributes<HTMLButtonElement>) =>
    <button data-element-key={elementKey} {...props}>{elementKey}</button>,
}))
vi.mock('@/components/composed/query-state/query-state', () => ({ QueryState: ({ children }: { children: ReactNode }) => children }))
