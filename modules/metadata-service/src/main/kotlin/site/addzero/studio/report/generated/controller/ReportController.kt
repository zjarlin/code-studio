...
private fun <T, R> PageResult<T>.mapRows(transform: (T) -> R): PageResult<R> = PageResult(
    rows = rows.map(transform),
    totalRowCount = totalRowCount,
    totalPageCount = totalPageCount,
)

private data class ReportHttpResponse(
    val status: HttpStatusCode,
    val body: CommonResult<Any?>,
    val cause: Throwable? = null,
)

private suspend fun runReportRequest(action: suspend () -> Any?): ReportHttpResponse = try {
    val data = action()
...
private suspend fun ApplicationCall.sendReportResponse(response: ReportHttpResponse) {
